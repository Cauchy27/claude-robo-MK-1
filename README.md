...
体感と数値が乖離する最大の理由: 本プラグインが圧縮するのは**アシスタント応答内の自然言語の地の文のみ**。コードブロック・ツール呼び出し・ファイル読み込み内容・履歴再送信などの入力トークン全般は保持対象で、一切圧縮されない。エージェンティックな作業では1ターンあたりの消費の大半は**入力側**が占めるため、地の文を3〜4割圧縮しても、それ単体ではセッション全体に占める割合は小さい。

### 入力側複利: 履歴再送信による増幅効果

一方で、上の$表が**計上していないプラス要因**がある。API はステートレスのため、ターン k の応答の地の文を Δ トークン削ると、以後の全ターンの入力（履歴再送信）でも毎回 Δ 効き続け、累積節約は `Δ × N(N−1)/2` —— **セッション長の二乗（O(N²)）で成長する**。ただし prompt caching がこの金額効果を約1/10に減衰させる（Claude Code はキャッシュ常用のため、再送信履歴の大半はキャッシュ読取単価≈0.1倍で課金）。
...
実環境では理論例（50ターンで1.5〜2倍）を大きく超え、**入力側複利が出力側の約15倍と支配項が逆転**。ただし超長セッションでは compaction が古い履歴を圧縮するため「750回再送」は上限値であり、$値も上限寄りの目安として読むこと。応答文単体の削減率（約3〜4割）がセッション全体では2〜3割に希釈されつつも、複利により金額インパクトは直接分の約10倍に増幅される——が本ケーススタディの結論。

### 週間利用上限（サブスクリプションプラン）の場合

Claude Pro/Max 等の週間上限付きプランでは、評価軸は「$節約額」ではなく「週間枠内でどれだけ余裕が増えるか」。入力・thinking・tool_use は非圧縮のため**方向としては効果は小さい側**と考えられるが、大きさは本リポジトリ側からは定量できない——効果は「地の文が週間消費全体に占める比率」に比例し、その内訳はユーザー環境・ワークロード依存のため。実際の利用状況は `/usage` 等で確認して判断すること。
...
**CLAUDE.md 直書きとの比較**: 直書きは毎ターン全文が input で累積消費大。プラグイン方式は SessionStart 1回注入 → cache 常駐で最適。SKILL.md は遅延読込、無効化は `/plugin disable` 一発、他ユーザーへの配布も可能。
...
📡 A Claude Code plugin that converts Japanese output into a **sci-fi AI robot tone** (HAL 9000 / GLaDOS style) — cold, logical, mechanical. Cuts response prose tokens by **roughly 30–40%** (measured) while prioritizing context fidelity. Japanese-focused; technical accuracy (code, API names, business logic) is fully preserved.
...
Fallback: `/plugin install claude-robo-MK-1@Cauchy27/claude-robo-MK-1`. Auto-enabled on new sessions (banner: `📡 claude-robo-MK-1 起動します`); use `/reload-plugins` for the current session. Verify via the **Installed** tab in `/plugin`.

### Style Modes

`/robo-hiragana` (default) / `/robo-katakana` / `/robo-mode`, or the `ROBO_STYLE` env var. Priority: `~/.claude-robo-mode` > `$ROBO_STYLE` > hiragana. Modes differ only in particles and verb endings.

### Effectiveness (summary)

> ⚠️ All dollar figures cover **only the natural-language prose inside responses**. Billed output also includes thinking and tool_use (which dominate agentic usage) — this plugin compresses none of that. Read the numbers as **upper-bound-leaning**.

- Measured prose reduction: **~29.4%** (n=5, range 17.1–35.1%; the auto-log's "40.0%" is a fixed assumption baked into `hooks/measure.sh`, not a measurement — see the Japanese section for the correction)
- Real-environment 90-day case study (2026-07-16, full logs): 7,432 responses; each response is resent as history on **~750 subsequent requests on average** (median session 80 turns, longest 5,024), so input-side compounding dominates at ~15× the output side. API-equivalent savings: conservative **~$3,000/90 days** (~$12,000/yr), upper bound $4,945 — against a total spend of ~$14,800/90d, i.e. **a 20–30% whole-session token reduction, 17–25% in cost terms**
- Compounding grows as `Δ × N(N−1)/2` (quadratic, not exponential); prompt caching dampens the dollar effect ~10×
- On weekly-cap subscription plans the effect direction is likely small and cannot be sized from this repo — check `/usage`
